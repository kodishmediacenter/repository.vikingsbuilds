...
#addon_data_dir = xbmc.translatePath("special/home/addons/plugin.video.playlistLoaderM/plugin.video.playlistLoaderM/data/").decode("utf-8")
...
def PlxCategory(url, cache):
	...
	for channel in chList[1:]:
		...
		if channel["type"] == 'playlist':
			AddDir("[COLOR Yellow]{0}[/COLOR]".format(name) ,channel["url"].encode("utf-8"), 1, iconimage, background=background.encode("utf-8"))
		else:
