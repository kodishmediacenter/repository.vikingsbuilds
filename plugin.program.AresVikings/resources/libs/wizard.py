...
def toggleAdult():
	do = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Voce Gostaria de? [COLOR %s]Ativar[/COLOR] ou [COLOR %s]Disable[/COLOR] Todo o Conteudo Adulto?[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR green]Ativar[/COLOR][/B]", nolabel="[B][COLOR red]Desativar[/COLOR][/B]")
...
def wizardUpdate(startup=None):
	if workingURL(WIZARDFILE):
		...
		zip = checkWizard('zip')
		if ver > VERSION:
			yes = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Existe uma nova versao do instalador [COLOR %s]%s[/COLOR] !' % (COLOR2, COLOR1, ADDONTITLE), 'Voce gostaria de Baixar e atualizar para a versao [COLOR %s]%s[/COLOR] ?[/COLOR]' % (COLOR1, ver), nolabel='[B][COLOR red]Lembre-me Mais Tarde[/COLOR][/B]', yeslabel="[B][COLOR green]Atualizar Agora![/COLOR][/B]")
			if yes:
...
def restoreLocal(type):
	...
	if file == "" or not file.endswith('.zip'):
		LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Restauracao Local: Cancelado[/COLOR]" % COLOR2)
		return
...
def clearCache(over=None):
	...
	LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Limpando Cache: Removido %s Arquivos[/COLOR]' % (COLOR2, delfiles))
